...
def test_clean(mock_clean, runner):
    ...
    assert result.exit_code == 0
    mock_clean.assert_called_once_with(verbose=False, clear_migration=True, clear_db=True)
...
def test_initialise(mock_upload, mock_collect, mock_run, mock_autoload, mock_clean, runner):
