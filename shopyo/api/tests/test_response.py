...
def test_error_response(app):
    with app.test_request_context():
        res, status = response.error_response(message="Bad Request", status=400, errors={"field": "required"})
        assert status == 400
        json_data = res.get_json()
...
def test_paginated_response(app):
    with app.test_request_context():
        items = [1, 2, 3]
        res, status = response.paginated_response(items=items, total=10, page=1, per_page=3)
        assert status == 200
        json_data = res.get_json()
        assert json_data["success"] is True
