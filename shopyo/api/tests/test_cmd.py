...
from shopyo.api.cmd import (
    clean, create_module, create_box, createmodulebox, 
    initialise, collect_static, create_module_in_box
)
...
def test_clean_cmd(mock_app):
    ...
    mock_db = MagicMock()
    with patch('shopyo.api.cmd.db', mock_db), \
         patch('shopyo.api.cmd.tryrmcache') as mock_rmcache, \
         patch('shopyo.api.cmd.tryrmfile') as mock_rmfile, \
         patch('shopyo.api.cmd.tryrmtree') as mock_rmtree:
