...
def test_startapp_integration(tmp_path, repo_root):
    ...
    result = subprocess.run(cmd_new, cwd=tmp_path, env=env, capture_output=True, text=True)
    ...
    result = subprocess.run(cmd_startapp, cwd=project_dir, env=env, capture_output=True, text=True)
    assert result.returncode == 0, f"shopyo startapp failed: {result.stdout} {result.stderr}"
...
"""
    ...
    result = subprocess.run(cmd_verify, cwd=project_dir, env=env, capture_output=True, text=True)
    
    assert "SUCCESS" in result.stdout, f"Verification failed:\nSTDOUT:\n{result.stdout}\nSTDERR:\n{result.stderr}"
