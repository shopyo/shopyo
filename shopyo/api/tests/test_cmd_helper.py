...
from shopyo.api.cmd_helper import (
    _clean, _collectstatic, _upload_data, _create_box, _create_module,
    _verify_app, _verify_box, _audit, _rename_app, _run_app
)
...
def test_clean(mock_app):
    with patch('shopyo.api.cmd_helper.current_app', mock_app), \
         patch('shopyo.api.cmd_helper.tryrmfile') as mock_rmfile, \
         patch('shopyo.api.cmd_helper.tryrmcache') as mock_rmcache, \
         patch('shopyo.api.cmd_helper.tryrmtree') as mock_rmtree:
...
def test_collectstatic(mock_get_folders, mock_rmtree, mock_exists):
    ...
    with patch('shopyo.api.cmd_helper.importlib.import_module'), \
         patch('shopyo.api.cmd_helper.sys.exit') as mock_exit:
...
def test_create_box(mock_mkdir):
    with patch('builtins.open', create=True) as mock_open:
        _create_box('testbox')
        mock_mkdir.assert_called_once_with(os.path.join('modules', 'testbox'), verbose=False)
...
def test_verify_app(mock_path_exists):
    ...
    with patch('builtins.open', create=True), \
         patch('json.load', return_value=info_json):
            res = _verify_app('modules/testapp', [])
            assert res['path'] == 'modules/testapp'
            assert res['issues'] == []

def test_rename_app():
    with patch('shopyo.api.cmd_helper.os.rename') as mock_rename, \
         patch('shopyo.api.cmd_helper.path_exists', return_value=True), \
         patch('builtins.open', create=True), \
         patch('json.load', return_value={'module_name': 'old'}), \
         patch('json.dump'):
